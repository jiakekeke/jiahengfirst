...
function move(obj,json,fn){  	// 属性值可以不加px 透明度是用0-1之间的数值obj要移动的对象而json是传入的移动总长度以及图片
    clearInterval(obj.timer)	// 清除正在执行的定时器

    var iSpeed  = 0;//运动速度

    obj.timer = setInterval(function(){//给移动对象设置个定时器
        var bOver = true; //假设运动完成
        for (var attr in json){  //每个属性的遍历修改
            var iCur = 0;
            var iTarget;     //   因为我们传入的透明度参数是0-1之间，而我们的初始值是乘100的，所以目标值应该也相应的乘100
            if(attr == "opacity"){	//透明度
                iCur = parseFloat(getStyle(obj,"opacity"))*100;//移动对象和opacity的值乘100
